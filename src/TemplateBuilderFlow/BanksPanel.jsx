    ...
    <div ref={ref} style={{
      ...
      {tab === "sections" && (
        <div style={{ padding: "10px 12px" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: C.g4, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>Category</div>
      ...
      <div style={{ padding: tab === "sections" ? "0 12px 10px" : "10px 12px" }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: C.g4, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>Tags</div>
          ...
          {TAGS_FILTER.map(tag => {
            ...
            return (
              <button key={tag} onClick={() => toggleTag(tag)}
                style={{
                  fontSize: 11, fontWeight: 500, fontFamily: F, borderRadius: 4, padding: "2px 7px",
...
function SectionBankCard({ section, onAdd }) {
  ...
  return (
    ...
    >
      {/* Top row: grip + name + category pill + add button */}
      <div style={{ display: "flex", alignItems: "flex-start", gap: 6, marginBottom: 4 }}>
        <div style={{ color: C.g3, flexShrink: 0, marginTop: 1 }}><IconGrip /></div>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: C.g4, fontFamily: F, marginTop: 2 }}>
        ...
        <button
          ...
          style={{
            ...
            border: `1px solid #c7cff7`, borderRadius: 5, padding: "2px 8px",
      ...
      {/* Description */}
      <div style={{ fontSize: 11, color: C.g4, fontFamily: F, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", paddingLeft: 18, marginBottom: 3 }}>
        {section.description}
      </div>
      {/* Last updated */}
      <div style={{ fontSize: 11, color: C.g3, fontFamily: F, paddingLeft: 18 }}>
...
function QuestionBankCard({ question, onAdd }) {
  const [hover, setHover] = useState(false);
  ...
  return (
    ...
    >
      {/* Answer type badge + add button */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <div style={{ color: C.g3, flexShrink: 0 }}><IconGrip /></div>
...
export default function BanksPanel({ open, onToggle, sections, onAddSectionFromBank, onAddQuestionFromBank, onToast }) {
    ...
    <div style={{
      ...
      {!open && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "10px 0", gap: 8 }}>
          <button
            onClick={onToggle}
            title="Open Banks panel"
            style={{ background: "none", border: "none", cursor: "pointer", color: C.g4, padding: 6, borderRadius: 6, display: "flex" }}
          ...
          ><IconSidebar /></button>
          <button
            onClick={() => { setTab("sections"); onToggle(); }}
            title="Section Bank"
            style={{ background: tab === "sections" ? "#eef1ff" : "none", border: "none", cursor: "pointer", color: tab === "sections" ? C.navy : C.g4, padding: 6, borderRadius: 6, display: "flex" }}
            onMouseEnter={e => { e.currentTarget.style.background = C.g1; }}
            onMouseLeave={e => { e.currentTarget.style.background = tab === "sections" ? "#eef1ff" : "none"; }}
          ><IconSection /></button>
          <button
            onClick={() => { setTab("questions"); onToggle(); }}
            title="Question Bank"
            style={{ background: tab === "questions" ? "#eef1ff" : "none", border: "none", cursor: "pointer", color: tab === "questions" ? C.navy : C.g4, padding: 6, borderRadius: 6, display: "flex" }}
            onMouseEnter={e => { e.currentTarget.style.background = C.g1; }}
            onMouseLeave={e => { e.currentTarget.style.background = tab === "questions" ? "#eef1ff" : "none"; }}
          ><IconQuestion /></button>
        </div>
      )}
      ...
      {open && (
        <>
          {/* Header row */}
          <div style={{ padding: "12px 12px 0", display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
            <button
              onClick={onToggle}
              title="Collapse panel"
              style={{ background: "none", border: "none", cursor: "pointer", color: C.g4, padding: 5, borderRadius: 6, display: "flex", flexShrink: 0 }}
              onMouseEnter={e => { e.currentTarget.style.background = C.g1; e.currentTarget.style.color = C.g6; }}
              onMouseLeave={e => { e.currentTarget.style.background = "none"; e.currentTarget.style.color = C.g4; }}
            ><IconSidebar /></button>
            <span style={{ fontSize: 13, fontWeight: 700, color: C.g6, fontFamily: F, flex: 1 }}>Banks</span>
          </div>
          ...
          <div style={{ padding: "10px 12px 0", display: "flex", gap: 6, flexShrink: 0, position: "relative" }}>
            <input
              ...
              style={{
                ...
                border: `1px solid ${C.g2}`, borderRadius: 7,
              ...
              <>
                {filteredSections.length === 0 ? (
                  ...
                  filteredSections.map(sec => (
                    <SectionBankCard key={sec.id} section={sec} onAdd={onAddSectionFromBank} />
                  ))
                )}
              </>
            )}
              ...
              <>
                {filteredQuestions.length === 0 ? (
                  ...
                  filteredQuestions.map(q => (
                    <QuestionBankCard key={q.id} question={q} onAdd={onAddQuestionFromBank} />
                  ))
                )}
              </>
            )}
